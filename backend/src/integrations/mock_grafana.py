...
class MockGrafanaServer:
    ...
    def query_prometheus(self, query: str) -> Dict[str, Any]:
        ...
        elif "rendition_error" in query or "transcode" in query:
            result_data.append({
    ...
    def search_loki(self, query: str, limit: int = 10) -> Dict[str, Any]:
        ...
        elif media_env.state == StreamState.RECOVERED:
            logs = [
                f"{now - 20} [INFO] edge-proxy-mumbai-02: Route shifted successfully to secondary-origin-chennai",
                f"{now - 10} [INFO] transcoder-worker-chennai-01: 1080p60_hdr segments transcoding smoothly, error rate 0.01%",
                f"{now - 2} [INFO] player-telemetry-collector: Playback buffer ratios returned to baseline (0.4%)"
            ]
        else:
            logs = [
